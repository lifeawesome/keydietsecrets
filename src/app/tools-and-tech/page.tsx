// app/tools-and-tech/page.tsx
import { getArticlesByCategorySlug } from "@/lib/queries";
import Link from "next/link";

export default async function ToolsAndTechPage() {
  const articles = await getArticlesByCategorySlug("tools-and-tech");
    ...
    <section className="space-y-10">
      <header className="space-y-4">
        ...
        <p className="max-w-3xl text-lg leading-relaxed text-stone-600">
          Reviews and recommendations for tools and technology that can support
        ...
        {articles.length === 0 && (
          <div className="rounded-2xl bg-white p-12 text-center shadow-md ring-1 ring-stone-200/50">
            ...
            <p className="mt-2 text-sm text-stone-500">
              We&apos;re working on creating comprehensive, evidence-based
              content for you.
            </p>
          </div>
        )}
        {articles.map((article) => (
          ...
          >
            <h2 className="text-2xl font-semibold text-stone-900 transition-colors group-hover:text-emerald-600">
              <Link href={`/tools-and-tech/${article.slug.current}`}>
                {article.title}
              </Link>
            </h2>
            {article.excerpt && (
            ...
            <Link
              href={`/tools-and-tech/${article.slug.current}`}
