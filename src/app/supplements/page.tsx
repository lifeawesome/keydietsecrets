...
export default async function SupplementsPage() {
  const articles = await getArticlesByCategorySlug("supplements");
    ...
    <section className="space-y-10">
      <header className="space-y-4">
        ...
        <p className="max-w-3xl text-lg leading-relaxed text-stone-600">
          Evidence-based information about supplements that may support your
          weight loss journey.
        </p>
      </header>
        ...
        {articles.map((article) => (
          ...
          >
            <h2 className="text-2xl font-semibold text-stone-900 transition-colors group-hover:text-emerald-600">
              <Link href={`/supplements/${article.slug.current}`}>
                {article.title}
              </Link>
            </h2>
            {article.excerpt && (
            ...
            <Link
              href={`/supplements/${article.slug.current}`}
