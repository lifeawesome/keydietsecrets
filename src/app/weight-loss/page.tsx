...
export default async function WeightLossPage() {
  const articles = await getArticlesByCategorySlug("weight-loss");
    ...
    <section className="space-y-10">
      <header className="space-y-4">
        ...
        <p className="max-w-3xl text-lg leading-relaxed text-stone-600">
          Science-backed articles that explain how fat loss really works—without
          the fluff.
        </p>
      </header>
        ...
        {articles.map((article) => (
          ...
          >
            <h2 className="text-2xl font-semibold text-stone-900 transition-colors group-hover:text-emerald-600">
              <Link href={`/weight-loss/${article.slug.current}`}>
                {article.title}
              </Link>
            </h2>
            {article.excerpt && (
            ...
            <Link
              href={`/weight-loss/${article.slug.current}`}
